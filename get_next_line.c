...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/09/17 10:35:44 by hmarkoun          #+#    #+#             */
/*   Updated: 2025/11/11 17:36:51 by hmarkoun         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!ft_strchr(saved_buffer, '\n'))
	{
		...
		if (!temp_buffer)
			return (ft_free(&saved_buffer));
		saved_buffer = ft_read_and_join(fd, saved_buffer, temp_buffer);
		free(temp_buffer);
	}
	line = ft_line(saved_buffer);
	if (!line)
		return (ft_free(&saved_buffer));
	saved_buffer = ft_buffer_clean(saved_buffer);
	return (line);
}
